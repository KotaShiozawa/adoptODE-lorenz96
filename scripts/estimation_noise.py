...
def gen_dataset(
    ...
    signal_variance = np.nanvar(dataset_gt.ys["state"][mask])

    ys = {"state": np.full((n_sys, len_segs, D), np.nan)}
    ys["state"][mask_ys] = (
        dataset_gt.ys["state"][mask]
        + jax.random.uniform(
            key=jax.random.key(0),
            shape=(n_sys, len_segs, D),
            minval=-noise_to_signal * signal_variance,
            maxval=noise_to_signal * signal_variance,
        )[mask_ys]
    )
    ...
    y0_lower_bound = y0_lower_bound.at[mask_y0].set(
        y0_train["state"][mask_y0] - noise_to_signal * signal_variance
    )
