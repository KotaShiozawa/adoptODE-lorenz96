"""This code snipped is an adaption of some of the optimizers included
in JAX, such that they can naturally enforce boundaries on parameters
during optimization."""
...
def optimizer(
    ...
    Args:
        opt_maker: a function that returns an :code:`(init_fun, update_fun, get_params)`
    ...
    Returns:
        An :code:`(init_fun, update_fun, get_params)` triple of functions that work on
        ...
        to :code:`ParameterPytree (OptStatePytree ndarray)`, but may store the state
    ...
    def tree_opt_maker(*args, **kwargs):
        ...
        def tree_init(x0_tree, **kwargs):
            x0_flat, tree = tree_flatten(x0_tree)
            kwargs_flat = {}
            for key in kwargs.keys():
                if not kwargs[key] is None:
                    kwargs_flat[key] = tree_flatten(kwargs[key])[0]
            kwargs_list = []
            for i in range(len(x0_flat)):
                kwargs_list.append({})
                for key in kwargs_flat:
                    kwargs_list[-1][key] = kwargs_flat[key][i]
            initial_states = [
                init(x0, **kwargs_i) for x0, kwargs_i in zip(x0_flat, kwargs_list)
            ]
...
@optimizer
def adam_bounded(step_size, b1=0.9, b2=0.999, eps=1e-8):
    ...
    def init(x0, upper_bound=None, lower_bound=None):
        ...
        return x0, m0, v0, adjust

    def update(i, g, state):
        x, m, v, adjust_boundaries = state
        m = (1 - b1) * g + b1 * m  # First  moment estimate.
        v = (1 - b2) * jnp.square(g) + b2 * v  # Second moment estimate.
        mhat = m / (1 - jnp.asarray(b1, m.dtype) ** (i + 1))  # Bias correction.
        vhat = v / (1 - jnp.asarray(b2, m.dtype) ** (i + 1))
        x = x - step_size(i) * mhat / (jnp.sqrt(vhat) + eps)
        return adjust_boundaries(x), m, v, adjust_boundaries
...
def sgd_bounded(step_size):
    ...
    def update(i, g, state):
        x, adjust = state
        return adjust(x - step_size(i) * g), adjust
...
@optimizer
def momentum_bounded(step_size: Schedule, mass=1.0):
    ...
    def update(i, g, state):
        x, velocity, adjust = state
        velocity = mass * velocity + g
        x = x - step_size(i) * velocity
        return adjust(x), velocity, adjust
...
def build_cosine_restart(
    initial_learning_rate, first_decay_steps, alpha=0.0, t_mul=2.0, m_mul=1.0
):
    """Build a cosine restart learning rate scheduel"""

    def cosine_restart(step):
        completed_fraction = step / first_decay_steps

        if t_mul != 1.0:
            i_restart = np.floor(
                np.log(1.0 - completed_fraction * (1.0 - t_mul)) / np.log(t_mul)
            )
