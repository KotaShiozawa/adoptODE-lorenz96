...
def collect_files(
    ...
    for file in glob(
        ...
        for segment in range(data.segment.size):
            ...
            mnidx = mse_true.isel(segment=segment).mean(dim='n_sys').idxmin(dim='time').values[0]

            def func(x, a, b):
                return a * x + b

            fit_data = np.log(mse_true.isel(segment=segment).sel(time=slice(0, mnidx)).dropna('time')).mean(dim='n_sys')
            fit_err = np.log(mse_true.isel(segment=segment).sel(time=slice(0, mnidx)).dropna('time')).std(dim='n_sys').values
            fit_x = fit_data.time.values.flatten()

            if mnidx > segment * len_segs/100 + 0.01:
                popt, pcov = curve_fit(func, fit_x.flatten(), fit_data.values[0], p0=[-1.0, 13], sigma=fit_err[0])
            ...
            if mnidx < segment * len_segs/100 + len_segs/100-0.02:
                fit_data_2 = np.log(mse_true.isel(segment=segment).sel(time=slice(mnidx, 15.4)).dropna('time')).mean(dim='n_sys')
                fit_err_2 = np.log(mse_true.isel(segment=segment).sel(time=slice(mnidx, 15.4)).dropna('time')).std(dim='n_sys').values
                fit_x_2 = fit_data_2.time.values.flatten()
                popt_2, pcov_2 = curve_fit(func, fit_x_2.flatten(), fit_data_2.values[0], sigma=fit_err_2[0])
...
def plot_results():
    ...
    for i, len_segs in enumerate([35, 45, 55, 65, 75, 100]):
        ...
        for segment in total_df['segment'].unique():
            ...
            success_rate = len(segment_df[np.logical_and(segment_df['mse'] < middle_threshold, segment_df['type'] == 'true')])/denom
            success_rate_upper = len(segment_df[np.logical_and(segment_df['mse'] < upper_threshold, segment_df['type'] == 'true')])/denom
            success_rate_lower = len(segment_df[np.logical_and(segment_df['mse'] < lower_threshold, segment_df['type'] == 'true')])/denom
    ...
    dist_ax.legend(handles, ["0.65", "1.0"], title=r'$t_S$', bbox_to_anchor=(1.0, 1), loc='upper left')
