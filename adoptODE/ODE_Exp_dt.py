...
def _initial_step_size(fun, t0, y0, order, rtol, atol, f0):
    # Algorithm from:
    ...
    y1 = y0 + h0 * f0
    f1 = fun(y1, t0 + h0, first_guess_dt)
...
def _optimal_step_size(
    ...
    factor = jnp.maximum(
        1.0 / ifactor, jnp.minimum(err_ratio ** (1.0 / order) / safety, 1.0 / dfactor)
    )
...
def _odeint(func, rtol, atol, mxstep, y0, ts, *args):
    ...
    def scan_fun(carry, target_t):
        ...
        def body_fun(state):
            ...
            next_t = t + dt
            error_ratios = _error_ratio(next_y_error, rtol, atol, y, next_y)
            new_interp_coeff = _interp_fit_dopri(y, next_y, k, dt)
            dt = _optimal_step_size(dt, error_ratios)

            new = [i + 1, next_y, next_f, next_t, dt, t, new_interp_coeff]
            old = [i + 1, y, f, t, dt, last_t, interp_coeff]
            return map(partial(jnp.where, jnp.all(error_ratios <= 1.0)), new, old)
    ...
    f0 = func_(y0, ts[0], first_guess_dt)
    dt = _initial_step_size(func_, ts[0], y0, 4, rtol, atol, f0)
